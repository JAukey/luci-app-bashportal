...
local dir = uci:get("bashportal","base","portalpath")

m = Map("bashportal", translate("Bash Portal FileTransfer"), translate("Emerge Bash Portal Project."))
...
fu.template = "cbi/file_upload"
um = sul:option(DummyValue, "", nil)
um.template = "cbi/file_dvalue"

fdl = SimpleForm("download", translate("Download"), nil)
fdl.reset = false
fdl.submit = false
sdl = fdl:section(SimpleSection, "", translate("Download file :input dir/file path"))
fd = sdl:option(FileUpload, "")
fd.template = "cbi/file_download"
dm = sdl:option(DummyValue, "", nil)
dm.template = "cbi/file_dvalue"
...
local inits, attr = {}
for i, f in ipairs(fs.glob( dir .. "*")) do
	attr = fs.stat(f)
	if attr then
		inits[i] = {}
...
btnrm.write = function(self, section)
	local v = luci.fs.unlink(dir .. luci.fs.basename(inits[section].name))
...
btnis.template = "cbi/file_button"
...
btnis.write = function(self, section)
	local r = luci.sys.exec(string.format('opkg --force-depends install "%s"', dir..inits[section].name))
	form.description = string.format('<span style="color: red">%s</span>', r)
end

return m,ful,fdl,form
